// Alternating LED example with Cypress bootloader support
// Build: make -C boot-blink2
// Program to EEPROM: fx2tool -d 04b4:8614 -B program -f boot-blink2/boot-blink2.ihex
...
usb_ascii_string_c usb_strings[] = {
  ...
  [1] = "FX2 series Cypress-class bootloader with two blinking LEDs",
};
...
// Alternating LED timer interrupt - toggles every 500ms
volatile uint16_t timer_counter = 0;

void isr_TF0(void) __interrupt(_INT_TF0) {
  // Increment counter on each timer overflow
  timer_counter++;
  
  // Toggle LEDs every ~500ms
  // 48MHz / 12 = 4MHz timer clock, mode 0 = 13-bit (8192 counts)
  // Overflow every 2.048ms, so 500ms / 2.048ms ≈ 244 overflows
  if(timer_counter >= 244) {
    timer_counter = 0;
    // Alternate between PA0 and PA1
    if(PA0) {
      PA0 = 0;
      PA1 = 1;
    } else {
...
int main(void) {
  ...
  // Configure LED pins
  PA0 = 1;      // LED 1 starts on
  PA1 = 0;      // LED 2 starts off
  OEA = 0b11;   // Set PA0 and PA1 as outputs

  // Configure TIMER0 for alternating LEDs
  ...
  // Main loop handles USB bootloader requests while LEDs alternate
  while(1) {
    if(pending_setup)
